<template>
  <div class="flex items-center h-screen w-full  bg-gray-900">
    <div class="w-full bg-white rounded shadow-lg p-8 m-4 md:max-w-sm md:mx-auto">
      ...
      <form class="mb-4" @submit.prevent="authStore.register">
        <div class="mb-4 md:w-full">
          <label for="name" class="block text-xs mb-1">Name</label>
          <input class="w-full border rounded p-2 outline-none focus:shadow-outline" type="name" name="name"
            id="name" placeholder="Name" v-model="authStore.formData.name"  />

          <span v-if="authStore.errors.name" class="text-red-700">{{
